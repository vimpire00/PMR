...
void vectordot(vector<Int> *W, 
vector<Int> *x_s, int nn, vector<Int> *y_s)
{
	int c = 0;
	Int sum = 0;
	for (int k = 0; k < nn; k ++)
	{
		sum = sum + moduloPMAX((*W)[k] * (*x_s)[k]);
		sum = moduloPMAX(sum);
	}
	y_s->push_back(moduloPMAX(sum));
	// 	}
...
void sum_v(vector<Int> *v, int nn, vector<Int> *y_c)
{
	Int sum = 0;
	for (int k = 0; k < nn; k ++)
	{
		sum = moduloPMAX(sum + (*v)[k]);
	}
	y_c->push_back(sum);
		//	cout << i << " " << y_c[i] << endl;
}
