...
vector<py::bytes> client_chamlon(vector<string>* w_in,
  vector<uInt>* r_in ){
...
vector<py::bytes> client_precomputation_szy(vector<string>* w_in,
    vector<uInt>* r_in, vector<uInt>* v_in ){
 ...
 // create SEAL objects
  ...
  istringstream istream(stringstream::binary);
  ostringstream ostream(stringstream::binary);
  vector<Ciphertext> results1;
  Ciphertext sum_szy; // 存储计算结果
  // For every batch
  vector<py::bytes> results2;
  int vec_len=2;
  for(size_t k = 0; k < w_in->size() ; k=k+vec_len){
  // for(size_t i = 0; i < w_in->size() ; i++){
    // read encrypted w
    int i=0;
    istream.str("");
    ...
    evaluator.multiply_plain(encryptedW, encodedR);
    // evaluator.add(sum_szy, encryptedW);
    results1.push_back(encryptedW);
    encryptedW.save(ostream);
    // results2.push_back(py::bytes(ostream.str()));
    ostream.str("");
  }
  evaluator.add_many(results1,sum_szy);
  sum_szy.save(ostream);
  results2.push_back(py::bytes(ostream.str()));
  // sum_szy.save(ostream);
  // ostream.str("");
